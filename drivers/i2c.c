...
static void send_ack(struct i2c *i2c, uint32_t);
static uint8_t read_byte(struct i2c *i2c, uint32_t);

static void i2c_scl(struct i2c *i2c, uint32_t bit) {
    gpio_direction_output(i2c->scl, bit);
}

static void i2c_sda(struct i2c *i2c, uint32_t bit) {
    if (bit)
...
static uint32_t i2c_read_sda(struct i2c *i2c) {
    return gpio_get_value(i2c->sda);
}

/*-----------------------------------------------------------------------
 * Send a reset sequence consisting of 9 clocks with the data signal high
 * to clock any confused device back uint32_to an idle state.  Also send a
 ...
 */
static void send_ack(struct i2c *i2c, uint32_t ack) {
    i2c_scl(i2c, 0);
    udelay(5);
    i2c_sda(i2c, ack);
    udelay(5);
 ...
 */
static uint32_t write_byte(struct i2c *i2c, uint8_t data) {
    uint32_t j;
    uint32_t nack;
 ...
 */
static uint8_t read_byte(struct i2c *i2c, uint32_t ack) {
    int data;
    int j;
